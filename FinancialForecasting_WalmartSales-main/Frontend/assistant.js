  ...
  if (!session || !session.messages.length) {
    chatContainer.innerHTML = `
            <div class="welcome-message">
                <h1 class="welcome-title">Sales Assistant</h1>
                <p class="welcome-subtitle">
                  Your AI-powered sales companion. Get expert advice on sales strategies, lead generation, and closing deals.</p>
            </div>`;
...
async function fetchOllamaResponse(userInput, sessionId, thinkingMsg) {
  try {
    ...
    if (cancelFetch) return insertStoppedMsg(sessionId, thinkingMsg);

    const data = await res.json();
    const content = data.response?.toString() || "⚠️ No message";

    finalizeAssistantMsg(sessionId, thinkingMsg, content);
  } catch (err) {
    if (cancelFetch) return insertStoppedMsg(sessionId, thinkingMsg);
    console.error(err);
    finalizeAssistantMsg(
      sessionId,
      thinkingMsg,
      "❌ Failed to connect to the backend. Make sure FastAPI is running on http://localhost:8000."
    );
  }
}
